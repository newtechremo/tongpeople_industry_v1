...
| UI 요소 | 상태 | 컴포넌트 | 경로 |
|---------|------|----------|------|
| 페이지 레이아웃 | ✅ | 기존 Layout | `src/App.tsx` |
| 사이드바 | ✅ | `Sidebar` | `src/components/Sidebar.tsx` |
| 헤더 | ✅ | `Header` | `src/components/Header.tsx` |
| 아이콘 (lucide) | ✅ | ChevronDown, Plus, X, Camera 등 | `lucide-react` |
| 결재 라인 (편집) | ✅ | `ApprovalLineEditable` | `src/components/risk-assessment/detail/ApprovalLineEditable.tsx` |
| 수시 상세 페이지 | 🆕 | `AdhocAssessmentDetailPage` | - |
| 수시 전용 헤더 | 🆕 | `AdhocDetailHeader` | - |
| 빈도·강도 카드 | 🆕 | `FrequencyIntensityCard` | - |
| 조치 정보 섹션 | 🆕 | `ActionInfoSection` | - |
| 검토 내용 입력 | 🆕 | `ReviewContentInput` | - |
| 검토 태그 목록 | 🆕 | `ReviewTagList` | - |
| 선택 정보 섹션 | 🆕 | `OptionalInfoSection` | - |
| 태그 입력 필드 | 🆕 | `TagInputField` | - |
| 태그 뱃지 | 🆕 | `TagBadge` | - |
| 접이식 섹션 | 🆕 | `CollapsibleSection` | - |
| 미확인 근로자 섹션 | 🆕 | `UnconfirmedWorkerSection` | - |
| 추가 위험 요인 섹션 | 🆕 | `AdditionalRiskSection` | - |
| 추가 위험 요인 카드 | 🆕 | `AdditionalRiskCard` | - |
| 조치 결과 섹션 | 🆕 | `ActionResultSection` | - |
| 조치 결과 카드 | 🆕 | `ActionResultCard` | - |
| 추가 위험 요인 모달 | 🆕 | `AdditionalRiskModal` | - |
| 조치 결과 모달 | 🆕 | `ActionResultModal` | - |
| 사진 업로더 | 🆕 | `PhotoUploader` | - |
| 개인정보 동의 모달 | 🆕 | `PrivacyConsentModal` | - |
| 성공 메시지 모달 | 🆕 | `SuccessMessageModal` | - |
| 작업 공종 드롭다운 | 🆕 | `SubCategoryDropdown` | - |
...
**Response Schema:**
```tsx
interface AdhocAssessmentDetailResponse {
  id: string;
  title: string;
  status: 'in_progress' | 'completed';
  assessmentType: 'adhoc';
  approvers: Approver[];
  riskFactors: {
    id: string;
    description: string;
    ...
    reviews: { id: string; content: string; createdAt: string }[];
  }[];
  inspectionItems: { id: string; content: string }[];
  nearMissTags: { id: string; content: string }[];
  unconfirmedWorkers: {
    id: string;
    name: string;
    company: string;
  ...
  additionalRisks: {
    ...
    content: string;
    createdAt: string;
    createdBy: { name: string; company: string; position: string };
  }[];
  actionResults: {
    id: string;
    photoUrl?: string;
...
**클라이언트 함수:**
```tsx
async function fetchAdhocAssessmentDetail(id: string): Promise<AdhocAssessmentDetailResponse> {
...
async function addTag(
  ...
  const response = await apiClient.post(endpoints[type], { content });
  return response.data;
}
```

### 5.3 추가 위험 요인 (FormData)
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|:----:|------|
| photo | File | ❌ | 사진 파일 (jpg, png) |
| content | string | ✅ | 위험 요인 내용 |

**Response:** `201 Created`
```tsx
interface AdditionalRiskResponse {
  id: string;
  photoUrl?: string;
  content: string;
  createdAt: string;
...
): Promise<AdditionalRiskResponse> {
...
|------|------|:----:|------|
| photo | File | ❌ | 사진 파일 (jpg, png) |
| subCategoryId | string | ✅ | 작업 공종 ID |
| content | string | ✅ | 조치 결과 내용 |

...
| 아이콘 (lucide) | ✅ | Edit2, Search 등 | `lucide-react` |
| 결재 라인 설정 참조 | ✅ | `ApprovalLineSettings` | `src/components/settings/ApprovalLineSettings.tsx` |
| 상세 헤더 | ⚠️ | `AssessmentDetailHeader` | 수정 필요 (onEdit 추가) |
| 결재 라인 (편집) | 🆕 | `ApprovalLineEditable` | - |
| 결재자 수정 모달 | 🆕 | `ApproverEditModal` | - |
| 결재자 검색 | 🆕 | `ApproverSearchInput` | - |
| 결재자 관리 훅 | 🆕 | `useApproverManagement` | - |

---

## 3. 컴포넌트 명세

### 3.1 AssessmentDetailHeader (수정)
